...
import { isWeatherSource, locationCandidates, placeLabel, shouldShowAnswerEvidence, taskErrorKey } from "./monitor-presentation";
  ...
  useEffect(() => {
    for (const task of tasks ?? []) {
      ...
      if (previous && previous !== run.status && ['succeeded', 'failed', 'superseded'].includes(run.status)) {
        setAnnouncement(t(run.status === 'succeeded' ? 'monitorActionDone' : run.status === 'failed' ? taskErrorKey(run.errorCode) : 'monitorErrorConflict'));
  ...
  return <section className="monitor-page" aria-labelledby="monitor-title">
    ...
    <div className="monitor-grid">{tasks.map((task) => {
      ...
      return <article className="surface-card monitor-card" key={task.id} id={`monitor-card-${task.id}`} tabIndex={-1} aria-labelledby={`monitor-name-${task.id}`}>
        ...
        {working && <div className="notice monitor-progress" id={`monitor-working-${task.id}`} role="status"><p><strong>{t(durableWorking ? stageKey : 'monitorStarting')}</strong></p>{durableWorking && execution?.usesLocalAi && <p>{t('monitorLocalWork')}</p>}{durableWorking && execution && <p>{t('monitorExpectedDuration', { minimum: Math.max(1, Math.ceil(execution.expectedDurationSeconds / 150)), minutes: Math.max(1, Math.ceil(execution.expectedDurationSeconds / 60)) })}</p>}{durableWorking && <p>{t('monitorLeavePage')}</p>}</div>}
