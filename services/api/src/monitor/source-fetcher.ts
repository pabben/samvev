...
export function normalizeMonitorUrl(value:string):string {
  ...
  if(!['http:','https:'].includes(url.protocol)||!hostname||url.username||url.password||hostname.includes('%')||blockedNames.has(hostname)||hostname.endsWith('.metadata.google.internal')) throw new DomainError('AI_ENDPOINT_BLOCKED',422);
...
function fetchPinned(url:URL,target:{address:string;family:4|6},signal:AbortSignal):Promise<{status:number;headers:Record<string,string|undefined>;body:Buffer}>{
  return new Promise((resolve,reject)=>{
    ...
    const options:RequestOptions={method:'GET',headers:{accept:'text/html,application/xhtml+xml,application/pdf;q=0.9','user-agent':'Samvev-Monitor/1.0'},signal,lookup:((_h:string,_o:unknown,cb:(e:Error|null,a:string,f:number)=>void)=>cb(null,target.address,target.family)) as RequestOptions['lookup']};
  ...
  async fetch(rawUrl:string,options:{followLinkedPdf?:boolean}={}):Promise<SourceDocument>{
    ...
    try{
      ...
      for(let redirects=0;redirects<=MAX_REDIRECTS;redirects++){
        ...
        if(response.status>=300&&response.status<400&&response.headers.location){if(redirects===MAX_REDIRECTS)throw new DomainError('MONITOR_SOURCE_UNAVAILABLE',502);url=new URL(normalizeMonitorUrl(new URL(response.headers.location,url).toString()));continue;}break;
      }
      ...
      if(kind==='text/html'||kind==='application/xhtml+xml'){
        ...
        if(pdf){
          ...
          if(linked.status>=200&&linked.status<300&&linkedType==='application/pdf'){text=await pdfText(linked.body,controller.signal);kind='application/pdf';finalUrl=safePdf.toString();metadata=linked;}else text=htmlText(html);
        }else text=htmlText(html);
      ...
      return {finalUrl,contentType:kind==='application/pdf'?'application/pdf':'text/html',text:normalized,fingerprint:createHash('sha256').update(normalized).digest('hex'),...(metadata.headers.etag?{etag:metadata.headers.etag}:{}),...(metadata.headers['last-modified']?{lastModified:metadata.headers['last-modified']}:{})};
