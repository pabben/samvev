...
function safeDetails(error:unknown):Record<string,unknown>|null{
  if(!(error instanceof DomainError)||error.code!=='MONITOR_LOCATION_AMBIGUOUS'||!Array.isArray(error.details?.candidates))return null;
  const candidates=error.details.candidates.slice(0,5).flatMap((candidate)=>{if(!candidate||typeof candidate!=='object')return[];const value=candidate as Record<string,unknown>;if(typeof value.name!=='string'||!value.name||value.name.length>200)return[];return[{name:value.name,municipality:typeof value.municipality==='string'?value.municipality.slice(0,120):null,region:typeof value.region==='string'?value.region.slice(0,120):null}];});
...
export class MonitorExecutionQueue {
  ...
  private async failInterrupted():Promise<void>{await transaction(async(client)=>{
    ...
    for(const item of expired.rows){
      ...
      const interpretation=item.kind==='interpretation'?(await client.query<{outcome:'success'|'failed';error_code:string|null;created_at:Date}>(`SELECT outcome,error_code,created_at FROM monitor_tool_audits WHERE execution_id=$1 AND task_id=$2 AND task_revision=$3 AND phase='interpretation' ORDER BY created_at DESC,id DESC LIMIT 1`,[item.id,item.task_id,item.task_revision])).rows[0]:undefined;
