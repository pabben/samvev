...
import { MONITOR_AGENT_DEADLINE_MS, MONITOR_LEASE_MS, MonitorAgentRunner, type MonitorAgentResult, type MonitorExecutionObserver, type MonitorToolAttempt, type MonitorToolProvenance } from './agent-runner.ts';
...
async function writeInterpretationAudit(taskId:string,taskRevision:number,outcome:'success'|'failed',meta:InterpretationAuditMeta,error:unknown=null,executionId?:string):Promise<void>{const persistedProvenance=outcome==='success'?meta.provenance:[];await pool.query(`INSERT INTO monitor_tool_audits(task_id,task_revision,phase,outcome,error_code,ai_call_count,attempted_tool_count,tool_count,provenance,attempts,execution_id) SELECT $1,$2,'interpretation',$3,$4,$5,$6,$7,$8,$9,$10 WHERE EXISTS(SELECT 1 FROM monitor_tasks WHERE id=$1)`,[taskId,taskRevision,outcome,outcome==='failed'?safeInterpretationError(error):null,meta.aiCalls,meta.attemptedTools,persistedProvenance.length,JSON.stringify(persistedProvenance),JSON.stringify(meta.attempts),executionId??null]);}
  ...
  async interpret(actor:MonitorActor,id:string,expectedRevision:number,execution:Partial<{deadlineMs:number;leaseMs:number;expectedProvider:MonitorAgentResult['provider'];observer:MonitorExecutionObserver;executionId:string}>={}){
    ...
    try{
      ...
      try{outcome=await runInterpretation(prompt,effectiveTier,toolPlan.includes('weather.forecast'));}
        ...
        try{outcome=await runInterpretation(prompt,'strong',toolPlan.includes('weather.forecast'),[],budget,expectedProvider);effectiveTier='strong';await writeInterpretationQualityAudit(id,expectedRevision,reason,'routine','strong','succeeded',execution.executionId);}
        catch(strongError){await writeInterpretationQualityAudit(id,expectedRevision,reason,'routine','strong','failed',execution.executionId);throw strongError;}
      }
...
function assertOutputLocale(actual:Locale,expected:Locale):void{if(actual!==expected)throw new DomainError('AI_RESPONSE_INVALID',502);}
...
function localizedCompositeExtraction(output:string,instruction:string,opened:SourceDocument[],now:Date,locale:Locale){
  ...
  const events=parsed.events.map((event)=>{try{if(event.date<today)throw new DomainError('AI_RESPONSE_INVALID',502);const primary=anchorEvidence(event.evidence,opened);const supporting=(event.evidence.sources??[]).map((ref)=>anchorEvidence(ref,opened));if(!webDocuments.includes(primary.source)||!primary.claims.length||supporting.length!==1||supporting[0]!.source!==weather||!supporting[0]!.claims.length)throw new DomainError('AI_COMPOSITION_INVALID',502);const refs=[primary,...supporting];const [year,month,day]=event.date.split('-');const dateForms=[event.date,`${day}.${month}.${year}`,`${day}/${month}/${year}`];if(!dateForms.some((value)=>normalizedClaim(primary.quote).includes(normalizedClaim(value)))||!weather.evidenceDates?.includes(event.date))throw new DomainError('AI_COMPOSITION_INVALID',502);if(event.time!==null&&!refs.some((ref)=>normalizedClaim(ref.quote).includes(event.time!)))throw new DomainError('AI_COMPOSITION_INVALID',502);const summary=weather.text.split('\n').find((line)=>line.startsWith(`Forecast summary ${event.date}:`));const weatherPresentation=summary?localizedWeatherAnswer(summary,locale):undefined;if(!summary||!weatherPresentation||!supporting[0]!.claims.every((claim)=>supportedByEvidence(claim,summary)))throw new DomainError('AI_COMPOSITION_INVALID',502);for(const person of event.who)if(!compositeClaimSupported(person,refs))throw new DomainError('AI_COMPOSITION_INVALID',502);const activityClaim=primary.claims.find((claim)=>/\p{L}/u.test(claim)&&!event.who.some((person)=>normalizedClaim(person)===normalizedClaim(claim)));if(!activityClaim)throw new DomainError('AI_COMPOSITION_INVALID',502);const type=locale==='nb'?'Plan og vær':'Schedule and weather';const description=locale==='nb'?`Aktivitet fra planen: «${activityClaim}». ${weatherPresentation}`:`Activity from the schedule: “${activityClaim}”. ${weatherPresentation}`;const actions=[locale==='nb'?'Sjekk klær og utstyr som passer til aktiviteten og været.':'Check clothing and equipment appropriate for the activity and weather.'];const uncertainty=event.uncertainty===null?null:(locale==='nb'?'Samvev er usikker på deler av tolkningen. Kontroller kildene.':'Samvev is uncertain about parts of the interpretation. Check the sources.');return{...event,type,description,actions,uncertainty,evidence:{quote:primary.quote,sourceUrl:primary.sourceUrl,claims:primary.claims,sources:supporting.map(({quote,sourceUrl,claims})=>({quote,sourceUrl,claims}))}};}catch(error){if(error instanceof DomainError&&error.code==='AI_RESPONSE_INVALID')throw new DomainError('AI_COMPOSITION_INVALID',502);throw error;}});
  const {outputLocale:_outputLocale,...publicParsed}=parsed;const result=monitorExtractionSchema.parse({...publicParsed,events});const rawEvents=detailed.raw&&typeof detailed.raw==='object'&&Array.isArray((detailed.raw as any).events)?(detailed.raw as any).events:[];const explicitValues=rawEvents.filter((item:unknown)=>item&&typeof item==='object'&&Object.hasOwn(item,'confidence')).map((item:any)=>item.confidence).filter((value:unknown):value is number=>typeof value==='number');analysisQuality.set(result,{explicitConfidence:explicitValues.length>0,confidence:explicitValues.length?Math.min(...explicitValues):null});return result;
}
function assertCompleteCompositeEvidence(opened:SourceDocument[],now:Date):void{
  const weather=opened.find((source)=>source.evidenceKind==='weather'||source.contentType==='application/vnd.met.no.locationforecast+json');const webDocuments=opened.filter((source)=>source!==weather&&(source.evidenceKind==='web'||source.contentType==='text/html'||source.contentType==='application/pdf'));if(!webDocuments.length||webDocuments.some((web)=>web.evidenceComplete!==true)||!weather||weather.evidenceComplete!==true)throw new DomainError('AI_COMPOSITION_INVALID',502);const summary=weather.text.split('\n').find((line)=>line.startsWith('Forecast summary '));const values=summary?weatherSummaryParts(summary):undefined;if(!values||values.date<now.toISOString().slice(0,10)||values.minimum===null||values.maximum===null||values.precipitation===null||values.wind===null||values.conditions===null||!weather.evidenceDates?.includes(values.date)||!webDocuments.some((web)=>web.evidenceDates?.includes(values.date)||web.text.includes(values.date)))throw new DomainError('AI_COMPOSITION_INVALID',502);
}
...
export function answerFromAi(output:string,sources:SourceDocument|SourceDocument[],options:{locale?:Locale}={}){const opened=Array.isArray(sources)?sources:[sources];const locale=options.locale??'nb';const weatherFallback=structuredWeatherAnswer(opened,locale);assertModelLocaleContract(output,'answer',locale);let detailed;try{detailed=schemaMonitorJsonDetailed(output,modelAnswerSchema,'AI_RESPONSE_INVALID');}catch(error){if(weatherFallback)return weatherFallback;throw error;}const parsed=detailed.data;assertOutputLocale(parsed.outputLocale,locale);if(weatherFallback)return weatherFallback;const claims=parsed.evidence.claims??[];let source:SourceDocument|undefined;if(claims.length){const anchored=anchorEvidence({quote:parsed.evidence.quote,sourceUrl:parsed.evidence.sourceUrl,claims},opened);if(!claims.some((claim)=>normalizedClaim(claim)===normalizedClaim(parsed.answer)))throw new DomainError('AI_RESPONSE_INVALID',502);source=anchored.source;}else{const candidates=parsed.evidence.sourceUrl?[evidenceSource(opened,parsed.evidence.sourceUrl)].filter((value):value is SourceDocument=>Boolean(value)):opened;source=candidates.find((candidate)=>sourceContainsEvidence(candidate,parsed.evidence.quote)&&sourceContainsEvidence(candidate,parsed.answer)&&supportedByEvidence(parsed.answer,parsed.evidence.quote));}if(!source)throw new DomainError('AI_RESPONSE_INVALID',502);const result=monitorAnswerSchema.parse({version:1,answer:parsed.answer,evidence:{quote:parsed.evidence.quote,sourceUrl:source.publicEvidenceUrl??source.finalUrl},confidence:parsed.confidence,uncertainty:null});const raw=detailed.raw&&typeof detailed.raw==='object'?detailed.raw as Record<string,unknown>:{};analysisQuality.set(result,{explicitConfidence:Object.hasOwn(raw,'confidence'),confidence:typeof raw.confidence==='number'?raw.confidence:null});return result;}
