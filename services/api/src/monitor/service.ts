...
import { MONITOR_AGENT_DEADLINE_MS, MONITOR_LEASE_MS, MonitorAgentRunner, type MonitorToolAttempt, type MonitorToolProvenance } from './agent-runner.ts';
...
const modelEventSchema=monitorExtractionSchema.shape.events.element.extend({
  evidence:monitorExtractionSchema.shape.events.element.shape.evidence.extend({sourceUrl:z.string().url().max(2048).optional()}),
  confidence:z.number().min(0).max(1).default(0),uncertainty:z.string().trim().max(500).nullable().default(null)
});
...
export class MonitorService{
  ...
  async interpret(actor:MonitorActor,id:string,expectedRevision:number){
    ...
    let audit:InterpretationAuditMeta=emptyInterpretationAudit();const interpretationController=new AbortController();const interpretationDeadline=setTimeout(()=>interpretationController.abort(),MONITOR_AGENT_DEADLINE_MS);
    try{
      ...
      const prompt=`You configure a durable Samvev task. The approved root source is ${approvedSourceUrl}. Use web.open only if current source context is needed to understand the request; Samvev performs the network access. Never construct or guess a link: open only the root or an exact link returned by web.open, and use fetched text/headings directly when they contain the answer. Never claim that you lack web access. Return only JSON matching {"version":1,"resultKind":"answer","summary":string,"eventTypes":string[],"keywords":string[],"people":string[],"noticeDaysBefore":integer,"noticeLocalTime":"HH:MM","checkIntervalMinutes":integer}; resultKind must be exactly "events" or "answer". Use "events" only for dated events/reminders and "answer" for a requested current fact, headline or summary. Do not invent facts. For "first headline", use the first editorial heading in fetched document order, excluding the page title, navigation and section labels; relevance-ranked links are not document order. Instruction: ${JSON.stringify(row.instruction)}. Current defaults: notice ${row.notice_days_before} day(s) before at ${String(row.notice_local_time).slice(0,5)}, check every ${row.check_interval_minutes} minutes.`;
      const runInterpretation=async(inputPrompt:string,requireTool:boolean,seedDocuments:SourceDocument[]=[],limits:Partial<{maxTurns:number;maxToolExecutions:number}>={})=>{
        try{const outcome=await this.agent.run({householdId:actor.householdId,task:{operation:'plan',purpose:'monitor_interpretation',input:inputPrompt,modelTier:row.model_tier as AiModelTier,maxOutputTokens:1200,sources:[]},policy:row.provider_policy as MonitorProviderPolicy,rootUrl:approvedSourceUrl,requireTool,seedDocuments,signal:interpretationController.signal,...limits});audit=mergeInterpretationAudit(audit,interpretationAuditFromResult(outcome));return outcome;}
        catch(error){audit=mergeInterpretationAudit(audit,interpretationAuditFromError(error));throw error;}
      };
      let outcome=await runInterpretation(prompt,false);
      let rule;
      try{rule=interpretationFromAi(outcome.output);}
      catch(error){
        if(!(error instanceof DomainError)||!['MONITOR_INTERPRETATION_SCHEMA_INVALID','MONITOR_INTERPRETATION_SOURCE_REFUSAL'].includes(error.code)||outcome.attemptedToolCount!==0)throw error;
        ...
        outcome=await runInterpretation(retryPrompt,true,outcome.documents,{maxTurns:6,maxToolExecutions:6});
        rule=interpretationFromAi(outcome.output);
      }
...
export function extractionFromAi(output:string,sources:SourceDocument|SourceDocument[],now=new Date()){
  const parsed=schemaMonitorJson(output,modelExtractionSchema,'AI_RESPONSE_INVALID');const today=now.toISOString().slice(0,10);
  const opened=Array.isArray(sources)?sources:[sources];
  const events=parsed.events.map((event)=>{const candidates=event.evidence.sourceUrl?[evidenceSource(opened,event.evidence.sourceUrl)].filter((value):value is SourceDocument=>Boolean(value)):opened;const [year,month,day]=event.date.split('-');const dateForms=[event.date,`${day}.${month}.${year}`,`${day}/${month}/${year}`];const claims=[event.type,event.description,...event.actions,...event.who];const quote=normalizedClaim(event.evidence.quote);const source=candidates.find((candidate)=>event.date>=today&&sourceContainsEvidence(candidate,event.evidence.quote)&&dateForms.some((value)=>quote.includes(normalizedClaim(value)))&&!claims.some((value)=>!supportedByEvidence(value,event.evidence.quote))&&(event.time===null||quote.includes(event.time)));if(!source)throw new DomainError('AI_RESPONSE_INVALID',502);return{...event,evidence:{...event.evidence,sourceUrl:source.finalUrl}};});
  return monitorExtractionSchema.parse({...parsed,events});
}
export function answerFromAi(output:string,sources:SourceDocument|SourceDocument[]){const parsed=schemaMonitorJson(output,modelAnswerSchema,'AI_RESPONSE_INVALID');const opened=Array.isArray(sources)?sources:[sources];const candidates=parsed.evidence.sourceUrl?[evidenceSource(opened,parsed.evidence.sourceUrl)].filter((value):value is SourceDocument=>Boolean(value)):opened;const source=candidates.find((candidate)=>sourceContainsEvidence(candidate,parsed.evidence.quote)&&sourceContainsEvidence(candidate,parsed.answer)&&supportedByEvidence(parsed.answer,parsed.evidence.quote));if(!source)throw new DomainError('AI_RESPONSE_INVALID',502);return monitorAnswerSchema.parse({...parsed,evidence:{...parsed.evidence,sourceUrl:source.finalUrl}});}
