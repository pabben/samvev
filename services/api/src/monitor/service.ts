...
export function weatherScopeFromInstruction(instruction:string,dynamicDateFromEvidence=false):ApprovedWeatherScope|undefined{
  ...
  const lower=normalized.toLocaleLowerCase('nb');const timeWindow:ApprovedWeatherScope['timeWindow']=/\b(?:morgen\s+tidlig|om\s+morgenen|morning)\b/.test(lower)?'morning':/\b(?:ettermiddag(?:en)?|afternoon)\b/.test(lower)?'afternoon':/\b(?:kveld(?:en)?|evening)\b/.test(lower)?'evening':/\b(?:natt(?:en)?|night)\b/.test(lower)?'night':'all';
  if(dynamicDateFromEvidence)return{location,period:'date',timeWindow,dynamicDateFromEvidence:true};
  const iso=lower.match(/\b(\d{4}-\d{2}-\d{2})\b/)?.[1];if(iso)return{location,period:'date',date:iso,timeWindow};
  const written=lower.match(/\b(\d{1,2})\.\s*([\p{L}]+)\s+(\d{4})\b/u);if(written&&monthNumbers[written[2]!])return{location,period:'date',date:`${written[3]}-${monthNumbers[written[2]!]}-${written[1]!.padStart(2,'0')}`,timeWindow};
  return{location,period:/\b(?:i\s+dag|today)\b/.test(lower)?'today':'tomorrow',timeWindow};
}
...
function strictExtractionFromAi(output:string,sources:SourceDocument|SourceDocument[],now=new Date()){
...
type WeatherSummaryParts={date:string;place:string;minimum:number|null;maximum:number|null;precipitation:number|null;wind:number|null};
function weatherSummaryParts(summary:string):WeatherSummaryParts|undefined{
  const match=summary.match(/^Forecast summary (\d{4}-\d{2}-\d{2}): location (.+?); minimum temperature (-?\d+(?:\.\d+)?|unknown) C; maximum temperature (-?\d+(?:\.\d+)?|unknown) C; total precipitation (\d+(?:\.\d+)?|unknown) mm; maximum wind (\d+(?:\.\d+)?|unknown) m\/s;/);
  if(!match)return undefined;const number=(value:string):number|null=>value==='unknown'?null:Number(value);return{date:match[1]!,place:match[2]!,minimum:number(match[3]!),maximum:number(match[4]!),precipitation:number(match[5]!),wind:number(match[6]!)};
}
...
function structuredConditionalWeatherExtraction(instruction:string,opened:SourceDocument[],now:Date,locale:Locale){
  if(!isConditionalInstruction(instruction)||opened.length!==2)return undefined;const web=opened.find((source)=>source.evidenceKind==='web');const weather=opened.find((source)=>source.evidenceKind==='weather'||source.contentType==='application/vnd.met.no.locationforecast+json');if(!web||!weather)return undefined;
  const summary=weather.text.split('\n').find((line)=>line.startsWith('Forecast summary '));const date=summary?.match(/^Forecast summary (\d{4}-\d{2}-\d{2}):/)?.[1];const hasDate=(source:SourceDocument,value:string)=>Boolean(source.evidenceDates?.includes(value)||source.text.includes(value));if(!summary||!date||date<now.toISOString().slice(0,10)||!hasDate(web,date)||!hasDate(weather,date))return undefined;
  const normalized=instruction.normalize('NFKC').toLocaleLowerCase('nb');const thresholdMatch=normalized.match(/\b(?:temperatur(?:en)?(?:\s+går)?|temperature)(?:\s+(?:er|is))?\s+under\s+(-?\d+(?:[.,]\d+)?)\s*(?:grader|degrees?|°c?)?\b/);const minimum=summary.match(/minimum temperature (-?\d+(?:\.\d+)?) C/)?.[1];const rainRequested=/\b(?:regn|nedbør|rain|precipitation)\b/.test(normalized);const precipitation=summary.match(/total precipitation (\d+(?:\.\d+)?) mm/)?.[1];let matched:boolean|undefined;
  if(thresholdMatch&&minimum!==undefined)matched=Number(minimum)<Number(thresholdMatch[1]!.replace(',','.'));else if(rainRequested&&precipitation!==undefined)matched=Number(precipitation)>0;if(matched===undefined)return undefined;if(!matched)return monitorExtractionSchema.parse({version:1,events:[]});
  const webQuote=web.text.split('\n').map((line)=>line.trim()).find((line)=>line.includes(date)&&line.length>0&&line.length<=500);if(!webQuote)return undefined;const type=(web.headings??[]).map((value)=>value.trim()).find((value)=>value.length>0&&value.length<=80&&webQuote.includes(value))??webQuote.slice(0,80).trim();if(!type)return undefined;const weatherClaim=thresholdMatch?`minimum temperature ${minimum} C`:`total precipitation ${precipitation} mm`;const verified=strictExtractionFromAi(JSON.stringify({version:1,events:[{date,time:null,type,description:`${type}; ${weatherClaim}`,actions:[],who:[],evidence:{quote:webQuote,claims:[type],sources:[{quote:summary,claims:[weatherClaim]}]},confidence:1,uncertainty:null}]}),opened,now);
  const userClaim=thresholdMatch?(locale==='nb'?`minimumstemperatur ${localizedNumber(Number(minimum),locale)} °C`:`minimum temperature ${localizedNumber(Number(minimum),locale)} °C`):(locale==='nb'?`${localizedNumber(Number(precipitation),locale)} mm nedbør`:`${localizedNumber(Number(precipitation),locale)} mm precipitation`);return{...verified,events:verified.events.map((event)=>({...event,description:`${type}; ${userClaim}`}))};
}
export function extractionFromAi(output:string,sources:SourceDocument|SourceDocument[],now=new Date(),fallback?:{instruction:string;locale?:Locale}){try{return strictExtractionFromAi(output,sources,now);}catch(error){const opened=Array.isArray(sources)?sources:[sources];const deterministic=fallback?structuredConditionalWeatherExtraction(fallback.instruction,opened,now,fallback.locale??'nb'):undefined;if(deterministic)return deterministic;throw error;}}
...
export function answerFromAi(output:string,sources:SourceDocument|SourceDocument[],options:{locale?:Locale}={}){const opened=Array.isArray(sources)?sources:[sources];const locale=options.locale??'nb';const weatherFallback=structuredWeatherAnswer(opened,locale);let detailed;try{detailed=schemaMonitorJsonDetailed(output,modelAnswerSchema,'AI_RESPONSE_INVALID');}catch(error){if(weatherFallback)return weatherFallback;throw error;}const parsed=detailed.data;const candidates=parsed.evidence.sourceUrl?[evidenceSource(opened,parsed.evidence.sourceUrl)].filter((value):value is SourceDocument=>Boolean(value)):opened;const source=candidates.find((candidate)=>sourceContainsEvidence(candidate,parsed.evidence.quote)&&sourceContainsEvidence(candidate,parsed.answer)&&supportedByEvidence(parsed.answer,parsed.evidence.quote));if(!source){if(weatherFallback)return weatherFallback;throw new DomainError('AI_RESPONSE_INVALID',502);}if(weatherFallback&&/^Forecast(?: summary)?\b/.test(parsed.answer))return weatherFallback;const result=monitorAnswerSchema.parse({...parsed,evidence:{...parsed.evidence,sourceUrl:source.publicEvidenceUrl??source.finalUrl}});const raw=detailed.raw&&typeof detailed.raw==='object'?detailed.raw as Record<string,unknown>:{};analysisQuality.set(result,{explicitConfidence:Object.hasOwn(raw,'confidence'),confidence:typeof raw.confidence==='number'?raw.confidence:null});return result;}
