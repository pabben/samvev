  ...
  async interpret(actor:MonitorActor,id:string,expectedRevision:number){
    ...
    try{
      ...
      try{outcome=await runInterpretation(prompt,effectiveTier,toolPlan.includes('weather.forecast'));}
        ...
        const budget=reason&&effectiveTier==='routine'?remainingEscalationBudget(audit.aiCalls,audit.attemptedTools,MONITOR_AGENT_DEADLINE_MS-(Date.now()-interpretationStarted)):null;
        ...
        await writeInterpretationQualityAudit(id,expectedRevision,reason,'routine','strong','escalated');
        try{outcome=await runInterpretation(prompt,'strong',toolPlan.includes('weather.forecast'),[],budget,expectedProvider);effectiveTier='strong';await writeInterpretationQualityAudit(id,expectedRevision,reason,'routine','strong','succeeded');}
        catch(strongError){await writeInterpretationQualityAudit(id,expectedRevision,reason,'routine','strong','failed');throw strongError;}
      }
      if(route.reason)await writeInterpretationQualityAudit(id,expectedRevision,route.reason,row.model_tier as AiModelTier,route.tier,'selected');
      let rule;
