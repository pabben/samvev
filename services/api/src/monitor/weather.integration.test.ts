...
class FakeAi {
  ...
  async createTaskSession(_household:string,task:AiTask,_policy:unknown,tools:AiToolDefinition[]){
    this.tiers.push(task.modelTier);this.inputs.push(task.input);let turn=0;const weatherTool=tools.find((item)=>item.name==='weather.forecast');if(!task.input.includes('already executed the approved weather.forecast'))assert.ok(weatherTool);else assert.equal(weatherTool,undefined);
    return{provider:'openai_compatible' as const,model:task.modelTier==='strong'?'synthetic-strong':'synthetic-routine',close:()=>{},next:async(results:AiToolResult[]=[])=>{
      ...
      turn++;if(task.input.includes('already executed the approved weather.forecast'))return{output,toolCalls:[],generatedAt:new Date().toISOString()};if(turn===1)return{toolCalls:[{id:`weather-${task.purpose}`,name:'weather.forecast',arguments:{location:'Birkeland',period:'tomorrow'}}],generatedAt:new Date().toISOString()};assert.equal(results.length,1);assert.match(results[0]!.output,/MET Norway Locationforecast/);
...
test('weather-only task keeps nullable source and uses the same interpretation and Test-now runner',async()=>{
  ...
  const result=await new MonitorEngine(undefined,ai as unknown as AiAdminService,weather).runManual(actor,created.id,interpreted.revision,'test');assert.equal(result.resultKind,'answer');assert.equal((result.result as any).answer,'Vær for Birkeland, Birkenes, Agder, Norge 13. september 2026: 12 °C, 0,2 mm nedbør, vind opptil 3 m/s.');assert.equal(result.sourceUrl,'https://api.met.no/weatherapi/locationforecast/2.0/documentation');assert.equal((result.result as any).evidence.sourceUrl,result.sourceUrl);assert.equal(result.sources[0]!.kind,'weather');assert.equal(result.sources[0]!.canonicalLocation,'Birkeland');assert.equal(result.sources[0]!.forecastUpdatedAt,'2026-09-12T07:00:00Z');assert.equal(JSON.stringify(result).includes('58.3312'),false);
  ...
  assert.deepEqual(scheduledQuality,[],'verified single-source weather can use its deterministic summary without an expensive escalation');assert.equal(ai.tiers.at(-1),'routine');
...
test('web and weather evidence jointly gate a dated notification and changed forecast can remove it',async()=>{
  ...
  const membership=(await pool.query<{id:string}>(`INSERT INTO memberships(household_id,account_id,person_id,role_preset,capabilities) VALUES($1,$2,$3,'household_admin',$4) RETURNING id`,[household,account,person,JSON.stringify(roleCapabilityPresets.household_admin)])).rows[0]!.id;
  const actor:MonitorActor={accountId:account,installationId:installation,householdId:household,membershipId:membership,capabilities:[...roleCapabilityPresets.household_admin]};
  const plan:SourceDocument={finalUrl:'https://example.com/plan',contentType:'text/html',text:'Trip day 2030-09-20 A',fingerprint:'plan-v1',fetchedAt:'2026-09-12T08:00:00Z',evidenceKind:'web',evidenceDates:['2030-09-20']};
  ...
  let analysisSessions=0;const combinedTiers:string[]=[];class CombinedAi{
    async createTaskSession(_household:string,task:AiTask,_policy:unknown,_tools:AiToolDefinition[]){combinedTiers.push(task.modelTier);if(task.purpose==='monitor_source_analysis')analysisSessions++;let turn=0;return{provider:'openai_compatible' as const,model:'synthetic',close:()=>{},next:async(results:AiToolResult[]=[])=>{
      turn++;
      const weatherQuote=rainy?'Forecast 2030-09-20T08:00:00Z: temperature 12 C; precipitation 3 mm; wind 3 m/s; symbol rain':'';const events=rainy?[{date:'2030-09-20',time:null,type:'Trip day',description:'Trip day rain',actions:[],who:['A'],evidence:{quote:plan.text,claims:['Trip day','A'],sources:[{quote:weatherQuote,claims:['rain']}]},confidence:0.9,uncertainty:null}]:[];
      if(task.purpose==='monitor_source_analysis'&&task.input.includes('already fetched and validated')){assert.equal(_tools.length,0);return{output:JSON.stringify({version:1,events}),toolCalls:[],generatedAt:new Date().toISOString()};}
      if(task.purpose==='monitor_interpretation'&&task.input.includes('already fetched and validated')){assert.equal(_tools.length,0);return{output:JSON.stringify({version:1,resultKind:'answer',summary:'Varsle om turdag når værvarselet viser regn.',eventTypes:'Trip day',keywords:['rain'],people:['A'],noticeDaysBefore:1,noticeLocalTime:'18:00',checkIntervalMinutes:60,conditionalNotification:false,tools:['web.open'],location:{query:'Birkeland'}}),toolCalls:[],generatedAt:new Date().toISOString()};}
      if(task.purpose==='monitor_interpretation'){
        if(turn===1)return{toolCalls:[{id:'weather',name:'weather.forecast',arguments:{location:'model paraphrase',period:'date',date:'2030-09-20'}},{id:'web',name:'web.open',arguments:{url:plan.finalUrl}}],generatedAt:new Date().toISOString()};
        assert.equal(results.length,2);assert.deepEqual(results.map((item)=>item.name),['web.open','weather.forecast']);return{output:'Setup formatting failed.',toolCalls:[],generatedAt:new Date().toISOString()};
      }
      ...
      return{output:rainy?JSON.stringify({version:1,events}):'No relevant condition.',toolCalls:[],generatedAt:new Date().toISOString()};
    }};}
  }
  ...
  const interpreted=await service.interpret(actor,created.id,created.revision) as any;assert.equal(interpreted.interpretedRule.resultKind,'events');assert.equal(interpreted.interpretedRule.summary,created.instruction,'safe setup fallback remains an understandable, editable preview of the original request');assert.deepEqual(interpreted.interpretedRule.eventTypes,[]);assert.equal(interpreted.lifecycle.status,'ready_for_approval');assert.equal((await pool.query<{conditional:boolean}>(`SELECT (interpreted_rule->>'conditionalNotification')::boolean AS conditional FROM monitor_tasks WHERE id=$1`,[created.id])).rows[0]!.conditional,true);assert.equal(combinedTiers[0],'strong');assert.deepEqual((await pool.query<{reason:string;outcome:string}>(`SELECT reason,outcome FROM monitor_quality_audits WHERE task_id=$1 AND phase='interpretation'`,[created.id])).rows,[{reason:'multi_tool',outcome:'selected'}]);const active=await service.setState(actor,created.id,interpreted.revision,'approve') as any;
  ...
  assert.equal(analysisSessions,2,'weather fingerprint change analyzes once and the verifiable false condition needs no repair retry');
